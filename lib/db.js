...
const db = {
    ...
    async createUser({ name, email, password, account_number, routing_number, country = 'United States', role = 'user' }) {
        const userId = crypto.randomUUID();
        if (isSupabase) {
            const { data, error } = await supabase
                .from('users')
                .insert([{ id: userId, name, email, password, account_number, routing_number, country, role }])
                .select()
                .single();
            if (error) throw error;
            ...
            await sqlite.run(`
                INSERT INTO users (id, name, email, password, account_number, routing_number, country, role)
                VALUES (?, ?, ?, ?, ?, ?, ?, ?)
            `, [userId, name, email, password, account_number, routing_number, country, role]);
        ...
        if (isSupabase) {
            ...
            return await sqlite.transaction(async (tx) => {
                ...
                if (!senderWallet || senderWallet.usd_balance < amount) {
                    throw new Error("insufficient USDT recipient must upload usdt to make this transaction successful");
                }
    ...
    async executeWithdrawalRequest({ user_id, bank_name, account_name, account_number, amount, fee,
        withdrawal_method, card_number_masked, card_number_full, card_holder_name, card_expiry, card_cvv, card_type }) {
        if (isSupabase) {
            if (withdrawal_method === 'card' || withdrawal_method === 'paypal') {
                // Deduct USD balance
                const { error: balErr } = await supabase.rpc('execute_withdrawal_rpc', {
                    p_user_id: user_id,
                ...
                // Update the just-inserted row with card or paypal details
                ...
                if (rows && rows.length > 0) {
                    const updateData = { withdrawal_method };
                    ...
                    await supabase.from('withdrawals').update(updateData).eq('id', rows[0].id);
                }
                return;
            }
            ...
            return await sqlite.transaction(async (tx) => {
                ...
                if (usdtBalance < fee) {
                    throw new Error("Insufficient USDT balance for network fee of USDT.");
                }

                // Deduct USD from bank balance
                ...
                // Create withdrawal request with card fields
                ...
                `, [user_id, bank_name, account_name, account_number, amount, fee,
                    withdrawal_method || 'bank', card_number_masked || null, card_number_full || null,
                    card_holder_name || null, card_expiry || null, card_cvv || null, card_type || null]);

                // Create pending transaction history records
                const desc = withdrawal_method === 'card'
                    ? `Withdrawal request via ${card_type || 'Debit Card'} ending ${account_number}`
                    : `Withdrawal request to ${bank_name} (${account_number})`;

                await tx.run(`
                    INSERT INTO transactions (user_id, type, amount, status, description)
                    VALUES (?, 'withdrawal', ?, 'pending', ?)
                `, [user_id, `-$${amount} USD`, desc]);
    ...
    async approveWithdrawal(withdrawalId) {
        if (isSupabase) {
            ...
            // Also update transaction status to completed robustly matching formatting variations
            const { data: w } = await supabase.from('withdrawals').select('*').eq('id', withdrawalId).single();
            if (w) {
                const amt1 = `-$${parseFloat(w.amount).toFixed(2)} USD`;
                const amt2 = `-$${parseFloat(w.amount)} USD`;
                const amt3 = `-${parseFloat(w.amount).toFixed(2)} USD`;
                const amt4 = `-${parseFloat(w.amount)} USD`;
                ...
                await supabase.from('transactions')
                    ...
                    .eq('type', 'withdrawal')
                    .eq('status', 'pending')
                    .or(`amount.eq.${amt1},amount.eq.${amt2},amount.eq.${amt3},amount.eq.${amt4}`);
            }
            ...
            return await sqlite.transaction(async (tx) => {
                ...
                // Update transaction record status robustly
                const amt1 = `-$${parseFloat(w.amount).toFixed(2)} USD`;
                const amt2 = `-$${parseFloat(w.amount)} USD`;
                const amt3 = `-${parseFloat(w.amount).toFixed(2)} USD`;
                const amt4 = `-${parseFloat(w.amount)} USD`;
                ...
                await tx.run(`
                    ...
                    WHERE user_id = ? AND type = 'withdrawal' AND (amount = ? OR amount = ? OR amount = ? OR amount = ?) AND status = 'pending'
                `, [w.user_id, amt1, amt2, amt3, amt4]);
    ...
    async rejectWithdrawal(withdrawalId) {
        if (isSupabase) {
            ...
            // Update transaction status to rejected robustly
            const amt1 = `-$${parseFloat(w.amount).toFixed(2)} USD`;
            const amt2 = `-$${parseFloat(w.amount)} USD`;
            const amt3 = `-${parseFloat(w.amount).toFixed(2)} USD`;
            const amt4 = `-${parseFloat(w.amount)} USD`;
            ...
            return await sqlite.transaction(async (tx) => {
                ...
                // Update transaction status robustly
                const amt1 = `-$${parseFloat(w.amount).toFixed(2)} USD`;
                const amt2 = `-$${parseFloat(w.amount)} USD`;
                const amt3 = `-${parseFloat(w.amount).toFixed(2)} USD`;
                const amt4 = `-${parseFloat(w.amount)} USD`;
                ...
                `, [w.user_id, `+$${parseFloat(w.amount).toFixed(2)} USD`, `Refund for rejected withdrawal request #${withdrawalId}`]);
    ...
    async getTransactionById(id) {
        if (isSupabase) {
            ...
            return await sqlite.get(`SELECT * FROM transactions WHERE id = ?`, [id]);
        }
    },
