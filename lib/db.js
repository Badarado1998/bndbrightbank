const sqliteHelper = require('./sqlite');
...
// Active database engine for local files (MySQL pool or SQLite direct connection)
const sqlite = isMySQL ? mysql : sqliteHelper;
...
const db = {
    ...
    async createDepositRequest({ user_id, method_id, proof }) {
        if (isSupabase) {
            ...
            const amtStr = `+${method.usd_credit} USD / +${method.deposit_amount} ${method.coin_symbol || method.coins?.symbol}`;
            await supabase.from('transactions').insert([{
                user_id,
                type: 'deposit',
                amount: amtStr,
                status: 'pending',
                description: `Deposit via ${method.coin_symbol || method.coins?.symbol} address ${method.wallet_address}`
            }]);
